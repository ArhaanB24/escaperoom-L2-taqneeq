$(document).ready(function () {
  ...
  function trivia() {
    ...
    function sumScore(questions) {
      var index=int(localStorage.getItem("index"))
      index+=1;
      localStorage.setItem("index",index);

      return scoreAry[index-1];
    ...
    function checkAnswer(answer, qNum, questions) {
      if (answer == questions[qNum].a) {
        ...
        $("body").css("background-color", "green").delay(500).queue(function () {
          $(this).css("background-color", "").dequeue();
        });
        score1++
      } else {
        scoreAry.push(questions[qNum].correct);
        $("body").css("background-color", "red").delay(500).queue(function () {
          $(this).css("background-color", "").dequeue();
        });
      }
    }
    var scoreAry = ["959812228027", "989527122280", "801298279522", "222780959812"];
